...
Worker1 IP:       172.20.20.11
Worker2 IP:       172.20.20.12
...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "worker2" do |node|
    ...
    node.vm.network "private_network", ip: "172.20.20.12"
...
Run all commands in this section on `control-plane`, `worker1`, and `worker2`.
...
sudo apt-get install -y kubelet=1.35.6-1.1 kubeadm=1.35.6-1.1 kubectl=1.35.6-1.1
...
Set node IP on `control-plane`.

```bash
echo "KUBELET_EXTRA_ARGS=--node-ip=172.20.20.10" | sudo tee /etc/default/kubelet
...
Set node IP on `worker1`.

```bash
echo "KUBELET_EXTRA_ARGS=--node-ip=172.20.20.11" | sudo tee /etc/default/kubelet
sudo systemctl daemon-reload
sudo systemctl restart kubelet
```

Set node IP on `worker2`.

```bash
echo "KUBELET_EXTRA_ARGS=--node-ip=172.20.20.12" | sudo tee /etc/default/kubelet
sudo systemctl daemon-reload
sudo systemctl restart kubelet
```

Paste the join command from the control plane on both `worker1` and `worker2`.
